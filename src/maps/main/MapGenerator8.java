...
public class MapGenerator8 extends MapGenerator{
	...
	public void generateMap(ArkhamMap m) {
		...
		//And now a little extra off the last map.  Now... setpieces!
		
		//Setpiece 1	1	1	1
		int matchingSpots = 0;
		for(int i = 0; i < m.WIDTH; i++){
			for(int j = 0; j < m.HEIGHT; j++){
				if(m.getTile(i, j) == 13 && m.getTile(i + 1, j) == 7 && m.getTile(i + 2, j) == 7 && m.getSetPiece(i, j) == 0 && m.getSetPiece(i + 1, j) == 0 && m.getSetPiece(i + 2, j) == 0){
					matchingSpots++;
				}
			}
		}
		...
		if(matchingSpots == 0){
			...
			double choose = Math.random() * matchingSpots;
			for(int i = 0; i < m.WIDTH; i++){
				for(int j = 0; j < m.HEIGHT; j++){
					if(m.getTile(i, j) == 13 && m.getTile(i + 1, j) == 7 && m.getTile(i + 2, j) == 7 && m.getSetPiece(i, j) == 0 && m.getSetPiece(i + 1, j) == 0 && m.getSetPiece(i + 2, j) == 0){
						choose--;
						if(choose < 0){
							m.setSetPiece(i, j, 1);
							m.setSetPiece(i + 1, j, 1);
							m.setSetPiece(i + 2, j, 1);
							j = m.HEIGHT;
							i = m.WIDTH;
						}
					}
		...
		for(int i = 0; i < m.WIDTH - 2; i++){
			for(int j = 0; j < m.HEIGHT - 2; j++){
				if(m.getTile(i, j) == 16 && m.getTile(i + 1, j) == 16 && m.getTile(i + 2, j) == 16 && m.getTile(i, j + 1) == 16 && m.getTile(i + 1, j + 1) == 16 && m.getTile(i + 2, j + 1) == 16 && m.getTile(i, j + 2) == 16 && m.getTile(i + 1, j + 2) == 16 && m.getTile(i + 2, j + 2) == 16 && m.getRiver(i, j) == 0 && m.getRiver(i + 1, j) == 0 && m.getRiver(i + 2, j) == 0 && m.getRiver(i, j + 1) == 0 && m.getRiver(i + 1, j + 1) == 0 && m.getRiver(i + 2, j + 1) == 0 && m.getRiver(i, j + 2) == 0 && m.getRiver(i + 1, j + 2) == 0 && m.getRiver(i + 2, j + 2) == 0 && m.getSetPiece(i, j) == 0 && m.getSetPiece(i + 1, j) == 0 && m.getSetPiece(i + 2, j) == 0 && m.getSetPiece(i, j + 1) == 0 && m.getSetPiece(i + 1, j + 1) == 0 && m.getSetPiece(i + 2, j + 1) == 0 && m.getSetPiece(i, j + 2) == 0 && m.getSetPiece(i + 1, j + 2) == 0 && m.getSetPiece(i + 2, j + 2) == 0 && (m.getTile(i, j - 1) != 16 || m.getTile(i + 1, j - 1) != 16 || m.getTile(i + 2, j - 1) != 16 || m.getTile(i + 3, j) != 16 || m.getTile(i + 3, j + 1) != 16 || m.getTile(i + 3, j + 2) != 16 || m.getTile(i + 2, j + 3) != 16 || m.getTile(i + 1, j + 3) != 16 || m.getTile(i, j + 3) != 16 || m.getTile(i - 1, j + 2) != 16 || m.getTile(i - 1, j + 1) != 16 || m.getTile(i - 1, j) != 16)){
					matchingSpots++;
				}
			}
		}
			...
			for(int i = 0; i < m.WIDTH - 2; i++){
				for(int j = 0; j < m.HEIGHT - 2; j++){
					if(m.getTile(i, j) == 16 && m.getTile(i + 1, j) == 16 && m.getTile(i + 2, j) == 16 && m.getTile(i, j + 1) == 16 && m.getTile(i + 1, j + 1) == 16 && m.getTile(i + 2, j + 1) == 16 && m.getTile(i, j + 2) == 16 && m.getTile(i + 1, j + 2) == 16 && m.getTile(i + 2, j + 2) == 16 && m.getRiver(i, j) == 0 && m.getRiver(i + 1, j) == 0 && m.getRiver(i + 2, j) == 0 && m.getRiver(i, j + 1) == 0 && m.getRiver(i + 1, j + 1) == 0 && m.getRiver(i + 2, j + 1) == 0 && m.getRiver(i, j + 2) == 0 && m.getRiver(i + 1, j + 2) == 0 && m.getRiver(i + 2, j + 2) == 0 && m.getSetPiece(i, j) == 0 && m.getSetPiece(i + 1, j) == 0 && m.getSetPiece(i + 2, j) == 0 && m.getSetPiece(i, j + 1) == 0 && m.getSetPiece(i + 1, j + 1) == 0 && m.getSetPiece(i + 2, j + 1) == 0 && m.getSetPiece(i, j + 2) == 0 && m.getSetPiece(i + 1, j + 2) == 0 && m.getSetPiece(i + 2, j + 2) == 0 && (m.getTile(i, j - 1) != 16 || m.getTile(i + 1, j - 1) != 16 || m.getTile(i + 2, j - 1) != 16 || m.getTile(i + 3, j) != 16 || m.getTile(i + 3, j + 1) != 16 || m.getTile(i + 3, j + 2) != 16 || m.getTile(i + 2, j + 3) != 16 || m.getTile(i + 1, j + 3) != 16 || m.getTile(i, j + 3) != 16 || m.getTile(i - 1, j + 2) != 16 || m.getTile(i - 1, j + 1) != 16 || m.getTile(i - 1, j) != 16)){
						...
						if(choose < 0){
							m.setSetPiece(i, j, 2);
							m.setSetPiece(i + 1, j, 2);
							m.setSetPiece(i + 2, j, 2);
							m.setSetPiece(i, j + 1, 2);
							m.setSetPiece(i + 1, j + 1, 2);
							m.setSetPiece(i + 2, j + 1, 2);
							m.setSetPiece(i, j + 2, 2);
							m.setSetPiece(i + 1, j + 2, 2);
							m.setSetPiece(i + 2, j + 2, 2);
							j = m.HEIGHT;
							i = m.WIDTH;
						}
					}
		...
		for(int i = 0; i < m.WIDTH; i++){
			for(int j = 0; j < m.HEIGHT; j++){
				if(m.getTile(i, j) == 16 && (m.getTile(i, j + 1) == 2 || m.getTile(i, j + 1) == 7) && m.getRiver(i, j) == 0 && m.getRiver(i, j + 1) == 0 && m.getSetPiece(i, j) == 0 && m.getSetPiece(i, j + 1) == 0){
					matchingSpots++;
				}
			}
		}
		...
		for(int i = 0; i < m.WIDTH - 2; i++){
			for(int j = 0; j < m.HEIGHT - 1; j++){
				if(m.getTile(i, j) == 16 && m.getTile(i + 1, j) == 16 && m.getTile(i + 2, j) == 16 && m.getTile(i, j + 1) == 16 && m.getTile(i + 1, j + 1) == 16 && m.getTile(i + 2, j + 1) == 16 && m.getRiver(i, j) == 0 && m.getRiver(i + 1, j) == 0 && m.getRiver(i + 2, j) == 0 && m.getRiver(i, j + 1) == 0 && m.getRiver(i + 1, j + 1) == 0 && m.getRiver(i + 2, j + 1) == 0 && m.getSetPiece(i, j) == 0 && m.getSetPiece(i + 1, j) == 0 && m.getSetPiece(i + 2, j) == 0 && m.getSetPiece(i, j + 1) == 0 && m.getSetPiece(i + 1, j + 1) == 0 && m.getSetPiece(i + 2, j + 1) == 0 && (m.getTile(i, j - 1) != 16 || m.getTile(i + 1, j - 1) != 16 || m.getTile(i + 2, j - 1) != 16 || m.getTile(i + 3, j) != 16 || m.getTile(i + 3, j + 1) != 16 || m.getTile(i + 2, j + 2) != 16 || m.getTile(i + 1, j + 2) != 16 || m.getTile(i, j + 2) != 16 || m.getTile(i - 1, j + 1) != 16 || m.getTile(i - 1, j) != 16)){
					matchingSpots++;
				}
			}
		}
			...
			for(int i = 0; i < m.WIDTH - 2; i++){
				for(int j = 0; j < m.HEIGHT - 1; j++){
					if(m.getTile(i, j) == 16 && m.getTile(i + 1, j) == 16 && m.getTile(i + 2, j) == 16 && m.getTile(i, j + 1) == 16 && m.getTile(i + 1, j + 1) == 16 && m.getTile(i + 2, j + 1) == 16 && m.getRiver(i, j) == 0 && m.getRiver(i + 1, j) == 0 && m.getRiver(i + 2, j) == 0 && m.getRiver(i, j + 1) == 0 && m.getRiver(i + 1, j + 1) == 0 && m.getRiver(i + 2, j + 1) == 0 && m.getSetPiece(i, j) == 0 && m.getSetPiece(i + 1, j) == 0 && m.getSetPiece(i + 2, j) == 0 && m.getSetPiece(i, j + 1) == 0 && m.getSetPiece(i + 1, j + 1) == 0 && m.getSetPiece(i + 2, j + 1) == 0 && (m.getTile(i, j - 1) != 16 || m.getTile(i + 1, j - 1) != 16 || m.getTile(i + 2, j - 1) != 16 || m.getTile(i + 3, j) != 16 || m.getTile(i + 3, j + 1) != 16 || m.getTile(i + 2, j + 2) != 16 || m.getTile(i + 1, j + 2) != 16 || m.getTile(i, j + 2) != 16 || m.getTile(i - 1, j + 1) != 16 || m.getTile(i - 1, j) != 16)){
						...
						if(choose < 0){
							m.setSetPiece(i, j, 4);
							m.setSetPiece(i + 1, j, 4);
							m.setSetPiece(i + 2, j, 4);
							m.setSetPiece(i, j + 1, 4);
							m.setSetPiece(i + 1, j + 1, 4);
							m.setSetPiece(i + 2, j + 1, 4);
							j = m.HEIGHT;
							i = m.WIDTH;
						}
					}
		...
		matchingSpots = 0;
		for(int i = 0; i < m.WIDTH; i++){
			for(int j = 0; j < m.HEIGHT - 4; j++){
				if(m.getTile(i, j) == 6 && m.getTile(i, j + 1) == 6 && m.getTile(i, j + 2) == 3 && m.getTile(i, j + 3) == 6 && m.getTile(i, j + 4) == 6 && m.getRiver(i, j) == 0 && m.getRiver(i, j + 1) == 0 && m.getRiver(i, j + 2) == 0 && m.getRiver(i, j + 3) == 0 && m.getRiver(i, j + 4) == 0 && m.getSetPiece(i, j) == 0 && m.getSetPiece(i, j + 1) == 0 && m.getSetPiece(i, j + 2) == 0 && m.getSetPiece(i, j + 3) == 0 && m.getSetPiece(i, j + 4) == 0){
					matchingSpots++;
				}
			}
		}
			...
			for(int i = 0; i < m.WIDTH; i++){
				for(int j = 0; j < m.HEIGHT - 4; j++){
					if(m.getTile(i, j) == 6 && m.getTile(i, j + 1) == 6 && m.getTile(i, j + 2) == 3 && m.getTile(i, j + 3) == 6 && m.getTile(i, j + 4) == 6 && m.getRiver(i, j) == 0 && m.getRiver(i, j + 1) == 0 && m.getRiver(i, j + 2) == 0 && m.getRiver(i, j + 3) == 0 && m.getRiver(i, j + 4) == 0 && m.getSetPiece(i, j) == 0 && m.getSetPiece(i, j + 1) == 0 && m.getSetPiece(i, j + 2) == 0 && m.getSetPiece(i, j + 3) == 0 && m.getSetPiece(i, j + 4) == 0){
						...
						if(choose < 0){
							m.setSetPiece(i, j, 5);
							m.setSetPiece(i, j + 1, 5);
							m.setSetPiece(i, j + 2, 5);
							m.setSetPiece(i, j + 3, 5);
							m.setSetPiece(i, j + 4, 5);
							j = m.HEIGHT;
							i = m.WIDTH;
						}
					}
		...
		for(int i = 0; i < m.WIDTH - 1; i++){
			for(int j = 0; j < m.HEIGHT; j++){
				if(m.getTile(i, j) == 16 && (m.getTile(i + 1, j) == 5 || m.getTile(i + 1, j) == 6) && m.getRiver(i, j) == 0 && m.getRiver(i + 1, j) == 0 && m.getSetPiece(i, j) == 0 && m.getSetPiece(i + 1, j) == 0){
					matchingSpots++;
				}
			}
		}
		...
		for(int i = 0; i < m.WIDTH; i++){
			for(int j = 0; j < m.HEIGHT; j++){
				if(m.getTile(i, j) == 7 && m.getRiver(i, j) == 0 && m.getSetPiece(i, j) == 0){
					matchingSpots++;
				}
			}
		}
